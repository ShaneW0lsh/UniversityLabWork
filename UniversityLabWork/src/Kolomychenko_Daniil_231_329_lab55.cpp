...
{
    std::cout << "Number of copy constructors calls currently is: " << ++number_of_calls << '\n';
}
...
{
    if (str[0] == '0' and calculate_length(str) == 1)
        return false;
    ...
    for (int i = 0; i < calculate_length(str); ++i) {
        if (i != 0 and (str[i] == '+' or str[i] == '-'))
            return false;
        if (!is_inside(str[i], approved_symbols))
            return false;
    }
    return true;
}
...
lab55::IntegerString lab55::IntegerString::operator-(const IntegerString& other) const
{
