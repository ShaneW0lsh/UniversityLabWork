...
{
    while (true) {
        std::cout << "Enter \"exit\" to exit or enter group number and name of "
                     "student to save it:\n";
        ...
        if (user_input == "exit") {
            ...
            std::vector<std::string> split_input = split(user_input);
            if (split_input.size() != 2) {
                std::cout << "Invalid input\n";
                continue;
            }

            students.emplace_back(split_input[0], split_input[1]);
        }
    }
}
